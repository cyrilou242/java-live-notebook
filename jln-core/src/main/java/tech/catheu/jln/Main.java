...
public class Main {
  ...
  public static class SharedConfiguration {
    public static final String AUTO_CLASSPATH = "JLN_AUTO_CLASSPATH";
    // notebook runtime configs
    @CommandLine.Option(names = {"-cp", "-classpath", "--class-path"},
                        paramLabel = "<CLASSPATH>",
                        description = "A : separated list of directories, JAR archives,\n and ZIP archives to search for class files. \nIf not set, jln looks for build system (eg maven) project files.")
    public String classPath = AUTO_CLASSPATH;

    @CommandLine.Option(names = {"--local-storage-path"}, paramLabel = "<PATH>",
                        description = "The fullpath to a folder to use as jln local storage, where extensions and states are cached.")
    public String localStoragePath = Paths.get(USER_HOME, ".java_live_notebook").toString();

    @CommandLine.Option(names = {"--no-utils"},
                        description = "If passed, disable the injection of jln-utils jar.")
